...
const REFUND_STEPS: {
...
}[] = [
  {
    ...
    label: "Requested",
    description: "Your refund has been submitted.",
  },
  {
  ...
  },
  {
    ...
    label: "Approved",
    description: "Refund approved. Processing soon.",
  },
  {
  ...
  },
  {
    ...
    label: "Completed",
    description: "Refund returned to your account.",
  },
];
    ...
    >
      ...
      {(requestedAt || processedAt) && (
        <div className="mt-5 pt-4 border-t border-gray-100 flex flex-wrap gap-4 text-xs text-gray-400">
          {requestedAt && (
            <span>Requested: {new Date(requestedAt).toLocaleString()}</span>
          )}
          {processedAt && (
            <span>Processed: {new Date(processedAt).toLocaleString()}</span>
          )}
        </div>
      )}
    </div>
