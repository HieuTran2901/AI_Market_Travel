...
function getStepStatus(
  ...
  if (currentStatus === PaymentStatus.FAILED && stepIndex === 2)
    return "failed";
  if (currentStatus === PaymentStatus.CANCELLED && stepIndex === 2)
    return "failed";
  if (currentStatus === PaymentStatus.EXPIRED && stepIndex === 2)
    return "failed";
  if (currentStatus === PaymentStatus.REFUNDED && stepIndex === 2)
    return "completed";

  if (stepIndex < currentIdx) return "completed";
  if (stepStatuses.includes(currentStatus)) return "current";
  if (stepIndex === 2 && currentStatus === PaymentStatus.SUCCESS)
    return "completed";
  return "upcoming";
}
...
const StepIcon: React.FC<{
  ...
  if (status === "completed") {
    return (
      <div className="w-9 h-9 rounded-full bg-emerald-500 flex items-center justify-center shadow-md shadow-emerald-200">
  ...
  if (status === "failed") {
    return (
      <div className="w-9 h-9 rounded-full bg-red-500 flex items-center justify-center shadow-md shadow-red-200">
          ...
          return (
            <React.Fragment key={step.label}>
              <div className="flex flex-col items-center flex-shrink-0">
                ...
                >
                  {status === "failed" && i === 2
                    ? (statusLabels[currentStatus] ?? "Failed")
                    : step.label}
                </span>
              </div>
              {!isLast && (
