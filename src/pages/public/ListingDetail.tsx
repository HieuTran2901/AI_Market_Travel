...
const ImageFallback = ({ title, className = '' }: { title: string; className?: string }) => (
  <div className={`flex h-full min-h-[220px] items-center justify-center bg-gradient-to-br from-blue-50 via-cyan-50 to-slate-100 text-center ${className}`}>
    <div>
...
const DetailGroupPanel = ({ group }: { group: DetailGroup }) => {
  const tone = toneStyles[group.tone];
  const Icon = group.icon;

  return (
    <div className={`motion-fade-up overflow-hidden rounded-3xl border bg-gradient-to-br p-5 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-slate-200/70 sm:p-6 ${tone.panel}`}>
      <div className="mb-5 flex items-start gap-4">
        <div className={`flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl shadow-lg ${tone.icon}`}>
          <Icon className="h-6 w-6" />
        </div>
        <div>
          <h3 className="text-lg font-bold text-gray-950">{group.title}</h3>
          <p className="mt-1 max-w-2xl text-sm leading-6 text-gray-500">{group.description}</p>
        </div>
      </div>
...
const CategoryDetailsSection = ({ listing, groups }: { listing: ListingResponse; groups: DetailGroup[] }) => (
  <div className="overflow-hidden rounded-3xl border border-blue-100 bg-white p-6 shadow-sm sm:p-8">
    <div className="mb-7 flex flex-col gap-4 border-b border-gray-100 pb-6 sm:flex-row sm:items-start sm:justify-between">
      <div className="max-w-2xl">
        <Badge className="bg-blue-50 text-blue-700 ring-1 ring-blue-100">
          {formatCategory(listing.category)} Essentials
        </Badge>
        <h2 className="mt-3 text-2xl font-bold text-gray-950">What to know before booking</h2>
        <p className="mt-2 text-sm leading-6 text-gray-500">
          Key details, inclusions, and traveler-friendly information organized for quick comparison.
        </p>
        <div className="mt-4 h-1 w-24 rounded-full bg-gradient-to-r from-blue-600 to-cyan-400" />
      </div>
      <div className="hidden h-14 w-14 items-center justify-center rounded-3xl bg-gradient-to-br from-blue-600 to-cyan-500 text-white shadow-lg shadow-blue-500/20 sm:flex">
    ...
    {groups.length > 0 ? (
      <div className="space-y-5">
        {groups.map(group => <DetailGroupPanel key={group.title} group={group} />)}
      </div>
    ) : (
      <StateBlock title="Details are being updated" description="The provider has not added category-specific information yet." className="border-dashed py-8" />
    )}
  </div>
);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50/50 to-white">
      <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <section className="motion-fade-up group relative mb-10 overflow-hidden rounded-3xl shadow-xl shadow-slate-200/80 ring-1 ring-black/5" style={{ animationDelay: '260ms' }}>
          {previewImages.length > 0 ? (
            <div className="grid h-[320px] gap-2 bg-white sm:h-[420px] lg:h-[500px] lg:grid-cols-[1.45fr_1fr]">
              <button
                type="button"
                className="group/photo relative overflow-hidden text-left focus:outline-none focus:ring-4 focus:ring-blue-500/30"
                ...
                {Array.from({ length: 4 }).map((_, index) => {
                  ...
                  return image ? (
                    <button
                      key={image.src}
                      type="button"
                      className="group/photo overflow-hidden focus:outline-none focus:ring-4 focus:ring-blue-500/30"
                    ...
                    <ImageFallback key={index} title={listing.title} />
                  );
                })}
              </div>
            </div>
          ) : (
            <ImageFallback title={listing.title} className="h-[320px] sm:h-[420px]" />
          )}
          {galleryImages.length > 0 && (
            <Button
              variant="secondary"
              className="absolute bottom-4 right-4 rounded-full bg-white/90 opacity-90 shadow-lg backdrop-blur transition-all duration-300 hover:-translate-y-0.5 hover:bg-white hover:opacity-100 hover:shadow-xl"
              onClick={() => openLightbox(0)}
            >
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start xl:grid-cols-[minmax(0,1fr)_380px]">
          <div className="min-w-0 space-y-8">
            <AnimatedSection delay={320} className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-3 max-w-3xl whitespace-pre-wrap text-base leading-8 text-gray-650">
                    {listing.description || listing.shortDesc || 'This listing is being prepared by the provider. Check back soon for richer travel details.'}
                  </p>
                </div>
              </div>
            </AnimatedSection>
            ...
            <AnimatedSection delay={380}>
              <div className="mb-4 flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-wide text-blue-600">Highlights</p>
                  <h2 className="mt-1 text-2xl font-bold text-gray-950">Why travelers choose this</h2>
                </div>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                {trustHighlights.map(item => (
                  <Card key={item.title} className="group/card rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                    <CardContent className="flex gap-4 p-5">
                      <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-600 to-cyan-500 text-white shadow-lg shadow-blue-500/20 transition-all duration-300 group-hover/card:scale-105 group-hover/card:shadow-blue-500/35">
            ...
            <AnimatedSection delay={440}>
              <CategoryDetailsSection listing={listing} groups={detailGroups} />
            </AnimatedSection>

            <AnimatedSection delay={500} className="grid gap-6 md:grid-cols-2">
              <Card className="rounded-3xl">
                <CardContent className="p-6">
                  <h2 className="flex items-center gap-2 text-xl font-bold text-gray-950">
                    <CheckCircle2 className="h-5 w-5 text-emerald-500" /> Amenities and inclusions
                  </h2>
                  {amenities.length > 0 ? (
                    <div className="mt-5 flex flex-wrap gap-2">
                      {amenities.map(item => (
                        <span key={item} className="rounded-full bg-emerald-50 px-3 py-1.5 text-sm font-medium text-emerald-700">{item}</span>
                      ))}
                    </div>
                  ) : (
                    <p className="mt-4 text-sm leading-6 text-gray-500">Amenities and inclusions vary by provider and booking date.</p>
                  )}
                </CardContent>
              </Card>
              ...
              <Card className="rounded-3xl">
                <CardContent className="p-6">
                  ...
                  <p className="mt-4 text-sm leading-6 text-gray-600">{location}</p>
                  <div className="mt-5 rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 to-cyan-50 p-5 text-sm font-medium text-blue-800">
            ...
            <AnimatedSection delay={560} className="rounded-3xl border border-amber-100 bg-amber-50/70 p-6 sm:p-8">
              <h2 className="flex items-center gap-2 text-xl font-bold text-gray-950">
                <ShieldCheck className="h-5 w-5 text-amber-600" /> Policies and important notes
              </h2>
              <div className="mt-5 grid gap-3 text-sm leading-6 text-gray-700 sm:grid-cols-3">
                <div className="rounded-2xl bg-white/75 p-4">Provider policies may vary by category and travel date.</div>
                <div className="rounded-2xl bg-white/75 p-4">Payment status and booking status are tracked in your account.</div>
                <div className="rounded-2xl bg-white/75 p-4">Refund eligibility depends on provider and platform review.</div>
              </div>
            </AnimatedSection>

            <AnimatedSection delay={620} className="rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-600 to-cyan-500 p-6 text-white shadow-xl shadow-blue-500/20 sm:p-8">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-wide text-blue-100">AI travel assistant</p>
                  <h2 className="mt-1 text-2xl font-bold">Ask how this fits your trip</h2>
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-blue-50">Compare this listing with your dates, budget, and travel style inside the AI planner.</p>
                </div>
                <Button className="rounded-full bg-white text-blue-700 transition-all duration-300 hover:-translate-y-0.5 hover:bg-blue-50 hover:shadow-lg" onClick={() => navigate('/ai/assistant')}>
          ...
          <aside className="motion-fade-up lg:sticky lg:top-24 lg:w-full" style={{ animationDelay: '420ms' }}>
